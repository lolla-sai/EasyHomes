<?php 
    ...
    {
        $r_id=$_POST['done'];
        $updater=mysqli_query($conn,"UPDATE books SET status=2 WHERE request_id=$r_id");
        if(mysqli_error($conn))
        {
            echo mysqli_error($conn);
        }
        else
        {
            ...
            $request=mysqli_fetch_assoc($get_request);
            $trucks=$request['no_of_trucks'];
            $category=$request['category_id'];
            $get_category=mysqli_query($conn,"SELECT * from pm_category WHERE category_id=$category");
            $cat=mysqli_fetch_assoc($get_category);
            $curr_trucks=$cat['no_of_trucks'];
            $total=$trucks+$curr_trucks;
            $update_category=mysqli_query($conn,"UPDATE pm_category SET no_of_trucks=$total WHERE category_id=$category");
            header('location:userpage.php');
        }
    }
...
<html>
    ...
    <div class="nav flex-column nav-pills me-3 p-3" id="v-pills-tab" role="tablist" aria-orientation="vertical">
